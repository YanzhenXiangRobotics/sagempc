...
class Agent(object):
    def __init__(self, my_key, X_train, origin, Cx_Y_train, Fx_Y_train, params) -> None:
        ...
        self.Fx_X_train = X_train.reshape(-1, self.env_dim - 1)
        self.Cx_X_train = X_train.reshape(-1, self.env_dim - 1)
    ...
    def get_lb_at_curr_loc(self):
        ...
        return (
            self.funct(
                torch.Tensor(self.current_location[:-1]).reshape(
                    -1, self.params["common"]["dim"] - 1
                )
            )
            .detach()
            .numpy()
    ...
    def __update_Cx_set(self, newX, newY):
        newX = newX.reshape(-1, self.env_dim - 1)
        newY = newY.reshape(-1, 1)
        self.Cx_X_train = torch.cat([self.Cx_X_train, newX]).reshape(
            -1, self.env_dim - 1
        )
    ...
    def get_optimistic_path(self, source, target):
        return nx.algorithms.shortest_paths.astar_path(
            self.optimistic_graph, source, target, heuristic=self.dist
        )
    ...
    def get_pessimistic_path(self, source, target):
        return nx.algorithms.shortest_paths.astar_path(
            self.pessimistic_graph, source, target, heuristic=self.dist
        )

    def update_pessimistic_graph(self, lower_bound, init_node, thresh, Lc):
        total_safe_nodes = torch.arange(0, lower_bound.shape[0])[lower_bound > thresh]
    ...
    def extract_optim_info(self, optim):
        self.safe_meas_loc = torch.Tensor(
            [optim.getx()[self.params["optimizer"]["Hm"] + 1], -2.0]
        ).reshape(-1, 2)
        reached = torch.norm(self.safe_meas_loc - self.planned_measure_loc) < 0.05
    ...
    def update_graph(self, Safe):
        ...
        # Order matters here
        self.update_pessimistic_graph(V_lower_Cx, Safe, self.constraint, self.Lc)
    ...
    def get_idx(self, positions):
        ...
        for position in positions:
            idx.append(torch.abs(torch.Tensor(self.V) - position).argmin().item())
        return idx
