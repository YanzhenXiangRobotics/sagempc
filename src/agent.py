...
class Agent(object):
    def __init__(
        ...
        self.planned_disk_center_at_last_meas = X_train.reshape(-1, self.env_dim)
    ...
    def get_lb_at_curr_loc(self):
        ...
        self.st_gp = "Cx"
        return (
            self.funct(torch.Tensor(self.current_location).reshape(-1, 2))
            .detach()
            .numpy()
        )
    ...
    def uncertainity_sampling(self, const_set="pessi"):
        ...
        curr_node = get_idx_from_grid(
            torch.from_numpy(self.current_location), self.grid_V
        )
    ...
    def __update_Cx_set(self, newX, newY):
        ...
        self.Cx_X_train = torch.cat([self.Cx_X_train, newX]).reshape(-1, self.env_dim)
