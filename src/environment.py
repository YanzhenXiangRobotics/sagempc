...
class ContiWorld:
    ...
    def __init__(self, env_params, common_params, visu_params, env_dir, params) -> None:
        ...
        env_file_name = env_params["env_file_name"]
        if self.block_env:
            env_file_name = "block_" + env_file_name
        ...
        if env_params["generate"] == True:
            global SingleTaskGP
            if self.block_env:
                self.__Cx_2D = self.block_true_constraint_sampling()
                assert (
                    self.VisuGrid.shape[0]
                    == self.__Cx_2D.shape[0] * self.__Cx_2D.shape[1]
                )
                self.__Cx = []
                for i in range(self.__Cx_2D.shape[0]):
                    for j in range(self.__Cx_2D.shape[1]):
                        self.__Cx.append(self.__Cx_2D[i, j])
            ...
            for init_agent in self.env_data["init_safe"]["loc"]:
                idx = torch.abs((self.VisuGrid - init_agent)[:, 0]).argmin()
    ...
    def block_true_constraint_sampling(self):
        self.Cx_X = (torch.rand(6) * 10).reshape(-1, self.env_dim)
        self.Cx_Y = torch.zeros(self.Cx_X.shape[0], 1)
        import os
        ...
        with open(os.path.join(dir_project, "block_world_dist.pt"), "rb") as f:
            min_dist_list = pickle.load(f)
        return torch.tensor(min_dist_list).T
