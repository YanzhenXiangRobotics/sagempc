...
class SEMPCNovaCarter(Node):
    ...
    def set_next_goal(self):
        ...
        if self.params["algo"]["objective"] == "SE":
            ...
            init_node = self.players[self.pl_idx].get_idx_from_grid(
                self.players[self.pl_idx].origin
            )
    ...
    def get_current_state(self):
        ...
        while not state_obtained:
            try:
                map_2_odom = self.tf_buffer.lookup_transform(
                    target_frame="map", source_frame="odom", time=rclpy.time.Time()
                )
                ...
                x_curr = np.append(x_curr, orient[2])
                state_obtained = True
            except:
                pass
        return x_curr
