...
def concat_const_val(ocp, params):
    ...
    ocp.constraints.lbx_e = np.concatenate([ocp.constraints.lbx_e, np.array([params["optimizer"]["Tf"]])])
    ocp.constraints.ubx_e = np.concatenate([ocp.constraints.ubx_e, np.array([params["optimizer"]["Tf"]])])
    ...
    ocp.constraints.ubx = np.concatenate([ocp.constraints.ubx, np.array([params["optimizer"]["Tf"]])])
...
def sempc_const_expr(model, x_dim, n_order, params, model_x, model_z, x_lin):
    ...
    ub_cx_lin = ca.SX.sym("ub_cx_lin")
    ub_cx_grad = ca.SX.sym("ub_cx_grad", x_dim, 1)
    if (
...
def export_sempc_ocp(params):
    ...
    if (
        ...
        if params["agent"]["dynamics"] == "NH_integrator":
            model = export_NH_integrator_ode_model_with_discrete_rk4(
                name_prefix + "sempc"
            )
