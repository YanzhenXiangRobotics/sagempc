...
class MPCRefTrackerNode(Node):
    ...
    def clock_listener_callback(self, msg):
        try:
            ...
            X, U = self.ctrl.solve_for_x0(np.concatenate((pose, self.velocity)))
    ...
    def plot(self, ref_path):
        ...
        ref_path_appended[: self.X_cl.shape[0], :] = self.X_cl.copy()
        ref_path_appended[ref_path.shape[0] : self.X_cl.shape[0], :] = ref_path[-1, :]

        _, ax = plt.subplots()
        ax.plot(
            ref_path_appended[:, 0], ref_path_appended[:, 1], marker="o", color="blue"
        )
...
if __name__ == "__main__":
    ...
    while tracker.iter < tracker.max_iter:
        rclpy.spin_once(tracker)
    tracker.plot(ref_path[:, :2])
